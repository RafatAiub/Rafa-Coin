...
class Block{
    ...
    mineBlock(difficulty){
        ...
        console.log("mining done successfully!!"+this.hash);
    }
    calculateHash(){
        return sha256(
            this.timestamp+
...
class Transaction{
    ...
    isValid(){
        ...
        const key = ec.keyFromPublic(this.formAddress,"hex");
        return key.varify(this.calculateHash(),this.signature);
    }
...
class BlockChain{
    ...
    addTransaction(transaction){
        if(!transaction.formAddress || !transaction.toAddress){
            throw new Error('Cannot Process tansaction');
        }
    ...
    isBlockChainValid(){
        for(let i = 1 ;i<this.chain.length;i++){
            ...
            if(!currentBlock.haveValidTransaction()){
                return false;
            }
        }
        return true;
